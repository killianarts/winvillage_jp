...
class Period:
    ...
    def set_left_date(self, date_):
        pass

    def set_range(self, date_range):
        pass

    def delete_session(self):
        pass

    def set_month(self, date_):
        pass

    def get_day_names(self, calendar_obj: calendar.Calendar) -> List[str]:
        """Utility function to generate day names"""
        return [calendar.day_abbr[day] for day in calendar_obj.iterweekdays()]

    def get_previous_month(self, date_):
        previous_month_date = date_ - relativedelta(months=1)
        return previous_month_date

    def get_next_month(self, date_):
        next_month_date = date_ + relativedelta(months=1)
        return next_month_date

    def get_localized_month_name(date_: date, locale="en"):
        return format_date(self, date_, "MMMM", locale=locale)

    def get_localized_day_names(self, firstweekday, locale="en"):
    ...
    def create_date_form(self, date_):
        form = DateForm(initial={"date": date_})
        return form

    def compile_dates_information(self, dates_iter):
        dates = []
        for date_ in dates_iter:
            form = create_date_form(date_)
            is_available = check_availability(date_)
            dates.append((date_, form, is_available))
        return dates

    def before_range_start(self, new_range: datetime, range_start: datetime):
...
def compile_dates_information(dates_iter):
    dates = []
    for date_ in dates_iter:
        form = create_date_form(date_)
        is_available = check_availability(date_)
